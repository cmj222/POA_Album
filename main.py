...
class WindowClass(QMainWindow, form_class):
    ...
    def __init__(self):
        ...
        # 리스트뷰시 파일들 목록들. 진입시에 초기화하고 다시 획득하는 과정 거침. 다음, 이전 버튼시에는 획득한 리스트의 파일들 안에서 순환
        self.imgsP = []
        self.imgsO = []
        self.imgsTa = []
        self.imgsJa = []
        ...
        # 텍스트 한번에 입력하는 기능에 사용할 함수들.
        self.text_from_anotherWindow = ''

        # 중간전달값용들...
        self.chosung = ''  # 자동추가기능의 중간변수용. 여기에 키워드가 초성으로 전환된 것이 저장되고 사용됨.
        ...
        # 첫실행여부 체크구간 #
        self.MakeDir()  # 같은 폴더에 필수폴더 체크하고 없을시 필수폴더 생성함수
        ...
        self.button_word_to_chosungs.clicked.connect(
            lambda state, situlation='from_word_edit': self.word_to_chosungs(situlation))
        ...
        self.word_edit.editingFinished.connect(
            lambda situlation='from_word_edit': self.word_to_chosungs(situlation))  # 단어창에 엔터치면 숫자란 자동입력
        ...
        self.window_text_input = None  # 현재 창이 떠지지 않은 상태임을 나타냄.

        # 테스트용버튼. 테스트 구문 삭제시 주석쳐서 비활성화 시킬것.
        #self.actionTest.triggered.connect(self.test1)
        ...
        # 미디어플레이어 오브젝트 만들기. mediaPlayer라는 이름으로.
        self.mediaPlayer_person = QMediaPlayer(None, QMediaPlayer.VideoSurface)
        self.mediaPlayer_object = QMediaPlayer(None, QMediaPlayer.VideoSurface)
        self.mediaPlayer_action = QMediaPlayer(None, QMediaPlayer.VideoSurface)

        # 비디오 위젯에 미디어플레이어를 연결해준다.
        self.mediaPlayer_person.setVideoOutput(self.video_person_widget)
        self.mediaPlayer_object.setVideoOutput(self.video_object_widget)
        self.mediaPlayer_action.setVideoOutput(self.video_action_widget)

        # 이후 viewPOA함수 안에서 이에 대한 추가함수를 넣는다.

    ### 첫실행인 경우 [ 필수폴더와 db파일 생성 ]
    ...
    # 폴더보기 버튼을 누른 경우 -> 좌상단의 탐색기창의 폴더를 새창에서 연다.
    def openMemoFolder(self):
        self.getSelected()
        path = os.path.realpath(self.crawler)
        os.startfile(path)

    # 폴더보기 눌렀는데 빈 폴더인 경우 -> 새로운이미지생성할꺼냐? 메시지박스
    ...
    # 폴더추가 버튼을 누른 경우 -> 현재 경로 읽고 우상단 스택을 3번째로 바꾼다.
    def addMemoFolder(self):
        self.getSelected()  # 현재 선택된 폴더를 경로포함이름으로 self.crawler에 반환
        if not self.crawler:  # 만약 선택된것이 없다면 걍 무반응되게
            return
        if self.image_o_addFolder.currentIndex() == 0:
            self.image_o_addFolder.setCurrentIndex(1)  # 스택위젯의 2번째로 전환한다.
        else:
            self.image_o_addFolder.setCurrentIndex(0)

    # 폴더추가스택에서 새로 추가 버튼을 누른경우 작동. 텍스트를 분리하고 그 수만큼 빈이미지파일 추가해서 연결시킨다.
    def addNewTextImage(self):
        if not self.crawler:  # 만약 선택된것이 없다면 걍 무반응되게
            return
        # 텍스트란의 텍스트 추출하고, 엔터를 기준으로 나눠서 리스트로 만든다.
        before_split = self.addFolder_textEdit.toPlainText()
        text_divby_enter = before_split.splitlines()  # 줄바꿈용 함수가 있구나 시발. 괜히 엔터인 /n 넣고 지랄했네.
        fileIndex = 1
        fileNumber = ''
        for line in text_divby_enter:
            # 파일인덱스를 가져다가 100이내의 경우라도 앞에 00이 올수있게 전환해서 텍스트넘버로 반환.
            if fileIndex < 10:
                fileNumber = '00' + str(fileIndex)
            elif fileIndex < 100:
                fileNumber = '0' + str(fileIndex)

            # 각 엔터마다 001.png... 파일을 만든다.
            filename = 'img' + fileNumber
            # 경로추가가 안되었잖나 시발 img001.png가 아니라 앞에 경로명.../ 이름이 나오게 해야지
            self.createNewImageFile(self.crawler, filename)

            fileIndex += 1
            if '\\' in line:
                # 각 엔터를 /로 나누고 앞의 것을 키워드에, 뒤에것을 추가설명으로 지정한다.
                keyword = line.split('\\')[0]
                # 에딧창이나 넘버창이 아닌 상황에서는 전달값 자체를 초성으로 전환하고 구한 값을 self.chosung으로 반환.
                self.word_to_chosungs(keyword)
                ...
                chosung = self.chosung
                memo = ''
            # json으로 저장하는 작업시작. 먼저 jsonM로 저장할 원본을 부른다.
            self.conf = JsonConfigFileManager('./PoaMemo.json')
            # 먼저 현재 현재의 파일이름으로 tagname 를 구한다.

            tagName = self.crawler + '/' + filename + '.png'
        ...
        # 텍스트입력창 초기화하기
        self.addFolder_textEdit.setPlainText('')
        # 추가입력모드에서 첫화면으로 돌아오게하기.
        self.addMemoFolder()
        # 보기모드 버튼 눌렀을때의 함수 실행해서 새로고침 반영하게 하기.
        self.viewPage()

    # 기존파일 있으면 거기에 텍스트를 분해해서 덮으는 식
    def useImage_addText(self):
        if not self.crawler:  # 만약 선택된것이 없다면 걍 무반응되게
            return
        # 텍스트란의 텍스트 추출하고, 엔터를 기준으로 나눠서 리스트로 만든다.
        before_split = self.addFolder_textEdit.toPlainText()

        # 텍스트를 엔터로 구별지어 리스트로 만들고 리스트의 길이를 구한다.
        text_divby_enter = before_split.splitlines()  # 줄바꿈용 함수가 있구나 시발. 괜히 엔터인 /n 넣고 지랄했네.
        len_text_div = len(text_divby_enter)

        #현재 경로를 겟하고 경로 내 [경로가 포함된 이름의]파일들의 리스트를 구한다 imgs[]. 그리고 그 길이를 구한다.len_imgs
        imgs = []
        self.getFolderImages(self.crawler, imgs)
        len_imgs = len(imgs)
        # 만약 텍스트리스트길이가 이미지리스트길이보다 짧다면 리스트[0]의 텍스트를 분해하여 imgs[0]에 대입을 반복
        # 한 줄에 \ 가 없는 경우 이를 무조거 나눠야한다고 생각해서 오류 내보냄...if로 나눠서 처리하게하자
        if len_text_div <= len_imgs:
            i = 0
            for line in text_divby_enter:
                if '\\' in line:
                    # 각 엔터를 /로 나누고 앞의 것을 키워드에, 뒤에것을 추가설명으로 지정한다.
                ...
                self.conf = JsonConfigFileManager('./PoaMemo.json')
                tagName = imgs[i]
                node1 = self.conf.values
                subsub = {}
                name = ['number', 'word', 'extra', 'memo']
                value = [chosung, keyword, '', memo]
                for x in range(len(name)):
                    subsub[name[x]] = value[x]
                node1[tagName] = subsub
                self.conf.values = node1
                self.conf.export('./PoaMemo.json')
                i += 1

        # 만약 반대라면 i 가 이미지들의 길이보다 작을때는 위처럼, 아닐때는 마지막 이미지파일명_i.png를 생성하고 대입
        elif len_text_div > len_imgs:
            i = 0
            while i < len_imgs: # 아직 이미지들이 남아있으니 텍스트리스트[i]를 imgs[i]에 대입시킨다.
                for line in text_divby_enter:
                    if '\\' in line:
                        # 각 엔터를 /로 나누고 앞의 것을 키워드에, 뒤에것을 추가설명으로 지정한다.
                        keyword = line.split('\\')[0]
                    ...
                    self.conf.export('./PoaMemo.json')
                    i += 1
            lastName = imgs[i]  # 앞의 경로까지 포함인데 이거...
            lastName = lastName.split('.')[0].split('/')[-1]  # 경로제외 확장자 제외 순수 이름 추출.
            while i > len_text_div:  # 이제 이미지 없으니 마지막 이름_i.png로 빈이미지 생성하고 붙인다
                line = text_divby_enter[i]
                filename = lastName + '_' + str(i).zfill(3)
                self.createNewImageFile(self.crawler, filename)
                keyword = line.split('\\')[0]
                self.word_to_chosungs(keyword)
                chosung = self.chosung
                memo = line.split('\\')[1]
                self.conf = JsonConfigFileManager('./PoaMemo.json')
                tagName = self.newImageFile  # 이게 맞는건가?
                node1 = self.conf.values
                subsub = {}
                name = ['number', 'word', 'extra', 'memo']
                value = [chosung, keyword, '', memo]
    ...
    # 위의 함수에서 img001이라는 변수 받아서 파일 생성한다.
    ...
    # 단어를 숫자로 전환하는 함수. 버튼을 누르거나 단어창에서 엔터를 치면 발동. 좌하단 입력란에 단어 입력 후 엔터시에도 발동.
    def word_to_chosungs(self, situlation):
        word = '초기값'
        ...
        # 개념을 부정하는 경우 ex 군국기무처폐지 를 ~군국기무처로 입력하게 한다. 그리고 초성전환시에는 생략하고 전환
        if '~' in word:
            word.lstrip('~')
        print('워드투넘버 함수의 변환할 텍스트는 ' + word)
        CHOSUNG_LIST = ['ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ']
        w_to_k = {'ㄱ': 'ㄱ', 'ㄲ': 'ㄱ', 'ㄴ': 'ㄴ', 'ㄷ': 'ㄷ', 'ㄸ': 'ㄷ', 'ㄹ': 'ㄹ', 'ㅁ': 'ㅁ', 'ㅂ': 'ㅂ',
                  'ㅃ': 'ㅂ', 'ㅅ': 'ㅅ', 'ㅇ': 'ㅇ', 'ㅈ': 'ㅈ', 'ㅉ': 'ㅊ', 'ㅊ': 'ㅊ', 'ㅋ': 'ㄱ', 'ㅌ': 'ㄷ', 'ㅍ': 'ㅍ',
                  'ㅎ': 'ㅎ'}
        chosung = []
        num = ''
        ...
        print('변환결과의 num은...' + num)
        self.chosung = num  # 여러줄 자동입력시 사용할 변수임... 아 시바 복잡해지네
    ...
    ### 작업할 수치확보
    # 숫자를 분석하고 p o a 값 반환하기. 숫자가 아니라 초성을 주게됨. 초성을 뽑겠지 뭐...
    def chosungs_to_P_O_A(self, number):
        print("나눠질 초성들은 " + number + "이다")
        length = len(number)
        if length >= 6:
            self.word_type = '6'
        ...
        if length == 3:
            ...
            self.person = number[0:2]
            self.object = number[1:3]
            self.Taction = number[2] + number[0]
            self.Jaction = ''
        if length == 2:
            self.word_type = '2'
            self.person = number[0:2]
            self.object = ''
            self.Taction = ''
            self.Jaction = number[0:2]

    # 하단의 gif 포함한 이미지를 p o a에 따라 적합한 곳에 표기하는 함수. 자주 활용됨.
    def showImageGifMp4(self, filename, stack, label):
        images = ["jpg", "png", "tga", "jpeg"]
        videos = ["avi", "mp4", "FLV", "MPEG", "WMV", "Ogg", "WebM"]

        # 경로포함 파일명을 . 으로 나눠서 리스트로 만들고 맨 마지막놈[= 확장자]를 가져온다.
        ext = str(filename).split('.')[-1]
        # 만약을 대비해서 확장자를 소문자로 전환시킨다.
        ext = ext.lower()

        # 입력받은 poa에 따라 해당하는 스택위젯을 비디오표기스택으로 바꾼다.
        # 출력할 비디오스크린이 어딘지 정해준다. 나중에 .setCurrentIndex(1)를 붙일것임.
        if stack == "p":
            stack_widget = self.stackedWidget_person
            video_screen = self.video_person_widget
            mediaPlayer = self.mediaPlayer_person
        if stack == "o":
            stack_widget = self.stackedWidget_object
            video_screen = self.video_object_widget
            mediaPlayer = self.mediaPlayer_object
        if stack == "a":
            stack_widget = self.stackedWidget_action
            video_screen = self.video_action_widget
            mediaPlayer = self.mediaPlayer_action

        # 미디어를 스톱시키는 과정이 필요한가? 이거 없으면 다음으로 넘겨도 계속 소리나오고 그러나?
        # ㅇㅇ 일단 스톱시켜라. 그래야 다음이 이미지일때 이미지가 뜨는데 소리는 그대로 나는 사태 방지.
        mediaPlayer.stop()

        # 비디오인 경우
        if ext in videos:
            # 스택위젯을 비디오표기하는 걸로 바꾼다.
            stack_widget.setCurrentIndex(1)

            # 미디어플레이어에 재생할 영상을 연결시켜준다.
            mediaPlayer.setMedia(
                QMediaContent(QUrl.fromLocalFile(filename)))
            # 재생하게 한다.
            mediaPlayer.play()

        elif ext in images:  # 만약 이미지확장자에 포함된다면
    ...
    def viewPOA(self, which):  # 어느 모드에서 시행하느냐에 따라 which에 eidtMode 혹은 viewMode가 옴.
        ...
        # 신호가 보기모드에서 왔는지, 편집모드에서 왔는지, 좌단 입력창에서 왔는지 체크.
        # 신호의 출처에 따라 어느 곳의 키워드를 가져올지 결정
        if which == 'editMode':
            ...
            if not gettext[0] in ['ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ']: # 입력란에 초성이 아닌 것이 입력되었다면...문자가 입력되었다면
                self.word_to_chosungs('from_POA_number')  # 입력란의 문자를 초성으로 전환한다.

        # 좌하단의 숫자를 입력값으로 하여 p o a를 반환하게 함.
        number = self.POA_number.text().replace(".", "")  # . 이 있는걸 처리할때는 씹도록 만든다.

        # 새로 진입하거나 새로고침인 상황이니 빈 리스트를 마련한다. 이미 존재하면 비운다.
        self.imgsP = []
        self.imgsO = []
        self.imgsTa = []
        self.imgsJa = []
        # 위의 리스트 중 몇번째를 볼지 정하는 순번을 초기화
        self.current_page_P = 0
        self.current_page_O = 0
        self.current_page_A = 0

        # 입력값의 길이를 토대로 self.person, object, Jaction, Taction 등에 값을 대입한다.
        self.chosungs_to_P_O_A(number)

        # 대입된 값을 토대로 원하는 이미지들이 담긴 폴더까지 경로를 구한다.
        self.pathP = str(self.PoaList) + '/' + self.person + 'p'
        ...
        # 5글자일때 오브젝트를 2~3번째 숫자의 인물로 대입하기 위한 변수 생성해둠.
        case5 = str(self.PoaList) + '/' + self.object + 'p'

        # 경로상의 이미지파일목록을 구하고 리스트에 추가.
        # 리스트가 비었으면 라벨에 '자료없음' 표기. 있으면 이미지,움짤,영상 표기

        # 아래의 포아라벨쇼 함수에 사용할 내부함수.
        # 입력값으로 인물사물행동 여부를 받는다. 나중에 쇼이미지움짤영상 함수에 계승하여 어느 스택위젯을 변환할지 전달
        # 입력값으로 경로와 리스트를 받는다. 경로의 파일들을 리스트에 담는다.
        # 입력값으로 표기할 라벨을 받는다. 쇼이미지움짤영상 함수에 계승하여 여기에 표기하게 만든다.
        def infunction(poa, PathPoa, ListPoa, label):
            List = []
            # 경로상의 이미지들을 경로포함이름으로 위의 빈리스트에 추가한다.
            self.getFolderImages(PathPoa, List)

            # 빈리스트에 담긴 내용물을 내보낼 리스트에 옮긴다. 리스트와 리스트는 대입이 안된다...
            for i in List:  # 주의!!! 리스트 = 리스트는 대입이 아니구나 시발. 대입하려면 이렇게 해야함.
                ListPoa.append(i)

            # 위 과정을 거쳤음에도 만약 리스트가 비어있다면 라벨있는 스택으로 전환하고 라벨을 '자료없음'으로 바꾼다.
            if not ListPoa:
                self.stackedWidget_person.setCurrentIndex(0)
                self.stackedWidget_object.setCurrentIndex(0)
                self.stackedWidget_action.setCurrentIndex(0)
                label.setText('자료없음')
            # 있다면 주어진 라벨에 이미지, 움짤, 영상을 표기한다.
            else:
                self.showImageGifMp4(ListPoa[0], poa, label)
            return ListPoa

        def POA_label_show():  # 아래의 poa의 3라벨 각각에 이미지를 표기한다.
            if str(self.word_type) in ['3', '6']:  # 조건문 주의사항...시발. == '3' or '5' 면 무조건 참이구나.
                infunction("p", self.pathP, self.imgsP, self.img_person_label)
                infunction("o", self.pathO, self.imgsO, self.img_object_label)
                infunction("a", self.pathTa, self.imgsTa, self.img_action_label)
            if str(self.word_type) == '5':  # 5글자일때 오브젝트를 2~3번째 숫자의 인물로 대입
                infunction("p", self.pathP, self.imgsP, self.img_person_label)
                infunction("p", case5, self.imgsO, self.img_object_label)  # 흠 이거 참...
                infunction("a", self.pathTa, self.imgsTa, self.img_action_label)
            elif str(self.word_type) in ['4', '2']:
                infunction("p", self.pathP, self.imgsP, self.img_person_label)
                self.img_object_label.setText('자료없음')
                infunction("a", self.pathJa, self.imgsJa, self.img_action_label)

        # 3이미지가 시험모드의 첫페이즈에서는 가려진 상태여야한다. 이를 처리하는 부분
        # 뷰모드이면서 동시에 보임상태라면...
        if which == 'viewMode' and self.showOrHidden == 'show':
            POA_label_show()
        ...
        self.viewPOA_show_number()    # poa이미지 리스트의 길이를 체크하여 "현재번호(=0) / 이미지개수" 를 표기.
        self.POA_number.clearFocus()  # 입력창의 자판입력 대기 상태를 해제
        self.word_edit.clearFocus()   # 입력창의 자판입력 대기 상태를 해제
    ...
    def viewPOA_next_pre(self, poa, isnext):
        ...
        if isnext == 'true':  # 다음버튼신호를 받음
            if not str(len(viewPOA) - 1) == str(self.current_page_poa):  # 매개페이지수와 매개페이지번호가 같지않으면
                ...
                self.showImageGifMp4(viewPOA[self.current_page_poa], stack, label)
                if poa == 'person':
